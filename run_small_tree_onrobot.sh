#!/bin/bash
# small_tree 装配 (OnRobot, onrobot_io = 工具口 pin16 开合: 0.0开/1.0关)。
# 产品: 黄2x2底 + 绿2x4中 + 绿2x2顶 (3块), 装配点=装配区原点。
# 布局见 src/panda_pick/src/final_product_small_tree.yaml。
...
ros2 run my_robot_vision run_auto_pick_pipeline.py \
  --product-yaml src/panda_pick/src/final_product_small_tree.yaml \
