...
bool driveGripperAction(rclcpp::Node::SharedPtr node, double pos) {
    auto client = rclcpp_action::create_client<FollowJT>(
        node, "/mj_panda_hand_controller/follow_joint_trajectory"
    );

    if (!client->wait_for_action_server(std::chrono::seconds(5))) {
        RCLCPP_ERROR(node->get_logger(), "夹爪 Action Server 未就绪");
        return false;
    }
    ...
    // 不要再 spin_until_future_complete，因为这个 node 已经在 executor 里 spin 了
    if (goal_handle_future.wait_for(std::chrono::seconds(2)) != std::future_status::ready) {
        RCLCPP_ERROR(node->get_logger(), "夹爪 goal 发送超时");
        return false;
    }

    auto goal_handle = goal_handle_future.get();
    if (!goal_handle) {
        RCLCPP_ERROR(node->get_logger(), "夹爪 goal 被拒绝");
        return false;
    }
    ...
    if (result_future.wait_for(std::chrono::seconds(3)) != std::future_status::ready) {
        RCLCPP_WARN(node->get_logger(), "等待夹爪结果超时，按已发送处理");
        return true;
    }

    auto wrapped_result = result_future.get();
    if (wrapped_result.code != rclcpp_action::ResultCode::SUCCEEDED) {
        RCLCPP_WARN(node->get_logger(), "夹爪动作未返回 SUCCEEDED，result code=%d",
                    static_cast<int>(wrapped_result.code));
        return false;
    }

    RCLCPP_INFO(node->get_logger(), "夹爪动作完成，目标开度=%.4f", pos);
    return true;
}
