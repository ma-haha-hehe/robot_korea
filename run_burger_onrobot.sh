#!/bin/bash
# burger 新夹爪版 (OnRobot, onrobot_io = 工具口 pin16 开合: 0.0开/1.0关)。
# 产品: 黄2x2底 + 黄2x4中 + 红2x2/蓝2x2并列 + 绿2x2顶 (5块), 装配点=装配区原点。
# 布局见 src/panda_pick/src/final_product_burger.yaml。
# 前提: UR 远程模式、已上电无保护停止、ur_robot_driver 在跑(不开 rviz)。
# ⚠️ descend 现为无夹爪深值(抓0.1993/放0.3671), 首跑手放急停旁, 或让我先调保守。
...
ros2 run my_robot_vision run_auto_pick_pipeline.py \
  --product-yaml src/panda_pick/src/final_product_burger.yaml \
