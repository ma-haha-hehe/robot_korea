...
本次公开范围是独立角度规划，入口不会启动机器人或物理执行。
本地研究适配器已调用同一核心；执行器集成仍需单独验收，未包含在本次发布中。
...
本地研究适配器的旧 `tasksh` 格式仍保留 `grasp_spin`（度）、`blueprint_yaw`（弧度）以及
...
逐任务角度规划证据见 [grasp-angle-report.json](validation/grasp-angle-report.json)。
